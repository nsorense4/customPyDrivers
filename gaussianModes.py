...
def HGBeam(l, m, x, y, A = 1, w = 30):
    I = A * hermite(l)(np.sqrt(2)*x/w) * hermite(m)(np.sqrt(2)*y/w) * np.exp(-(x**2 + y**2)/w**2)
...
#%% create NxM array of normalized LGlm mode. 
...
#%% create NxM array of normalized HGlm mode.

def HGModeArray(l,m,N=100, M=100, w=20):
    x = np.arange(-np.floor(N/2), np.ceil(N/2))
    y = np.arange(-np.floor(M/2), np.ceil(M/2))
    xx,yy = np.meshgrid(x, y, indexing='ij')
    
    field = HGBeam(l=l, m=m, x = xx, y=yy, w = w)
    return field/np.max(np.abs(field))

#%% Example: Plot the first l x m LG modes.
...
#%% Example: Plot the first l x m HG modes.
...
# for l in range(L):
#     for m in range(M):
#         field = HGBeam(l=l, m=m, x = xx, y = yy, w = N/5)
#         ax[l,m].imshow(np.abs(field)*0, cmap = 'hot')
#         ax[l,m].imshow(np.angle(field),  alpha = np.abs(field)**2/np.max(np.abs(field))**2, cmap = 'hsv', vmin = -np.pi, vmax = np.pi, interpolation = 'none')
